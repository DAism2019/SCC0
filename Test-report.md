...
async function SCC0LicenseManager_test(singer0,singer1,_owner0,_owner1){
  ...
  // 3. 添加新的正式许可证版本
    try {
      res=await manager0.addSCC0Version([_owner0,await scc01.getAddress(),5,true]);
      await res.wait();
      console.log('version5 is ok: '+res.hash);
      res=await manager0.addSCC0Version([_owner0,await scc03.getAddress(),6,true]);
      await res.wait();
      console.log('version6 is ok: '+res.hash);

      res=await manager0.addSCC0Version([_owner1,await scc02.getAddress(),5,true]);
      await res.wait();
      console.log('version5 is ok: '+res.hash);
      res=await manager0.addSCC0Version([_owner1,await scc04.getAddress(),6,true]);
      await res.wait();
      console.log('version6 is ok: '+res.hash);
    } catch (error) {
      console.log("版本号冲突")
    ...
    // 4. 添加废弃的协议版本
try {
  res=await manager0.addDeprecatedVersion(await scc01.getAddress());
  await res.wait();
...
try {
  res=await manager1.addDeprecatedVersion(await scc04.getAddress());
  await res.wait();
  console.log('addDeprecatedVersion is ok: '+res.hash);
...
// 5. 检查协议版本是否存在
 res=await manager0['isSCC0Version(uint256)'](5);
 console.log("version5:",res);
 res=await manager0['isSCC0Version(address)'](_owner0);
 console.log("version6",res);

//  6. 获取正式许可证信息
  res=await manager0['getSCC0Info(uint256)'](6);
  console.log(res);
  res=await manager0['getSCC0Info(address)'](await scc01.getAddress());
  console.log(res);

  // 7. 获取所有正式 SCC0 协议地址
  res=await manager0.getAllSCC0Versions();
  console.log('获取所有正式 SCC0 协议地址',res);
  
  // 8. 获取所有有效状态的许可证地址
  res=await manager0.getAllActivedVersions();
  console.log('获取所有有效状态的许可证地址',res);
